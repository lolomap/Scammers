...
internal sealed class Operation : Symbol
{
    ...
    private static Expression AbsExpression(Expression expression)
    {
    ...
    private static Expression GreaterThanOrEqualExpression(Expression a, Expression b)
    {
        BinaryExpression expression = Expression.GreaterThanOrEqual(a, b);
        return Expression.Call(null,
            typeof(Convert).GetMethod("ToDecimal", new[] {expression.Type})!, expression);
    }
    private static Expression LessThanExpression(Expression a, Expression b)
    {
        BinaryExpression expression = Expression.LessThan(a, b);
        return Expression.Call(null,
            typeof(Convert).GetMethod("ToDecimal", new[] {expression.Type})!, expression);
    }
    private static Expression LessThanOrEqualExpression(Expression a, Expression b)
    {
        BinaryExpression expression = Expression.LessThanOrEqual(a, b);
        return Expression.Call(null,
            typeof(Convert).GetMethod("ToDecimal", new[] {expression.Type})!, expression);
    }
    ...
    public static readonly Operation Multiplication = new(2, Expression.Multiply, "Multiplication");
